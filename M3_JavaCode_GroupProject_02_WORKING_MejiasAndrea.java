...
// ITEM CLASS - Tracks store inventory and sales
...
class Item {
    ...
    /**
     * Sells a given amount of this item
...
// SERVICE CLASS - Schedules and tracks services + payments
...
class Service {
    ...
    public void complete() {
        if (status == Status.SCHEDULED) status = Status.COMPLETED;
    }

    public void cancel() {
        if (status == Status.SCHEDULED) status = Status.CANCELLED;
    }
...
// ANIMAL CLASS - Manages farm-raised and breeder resale animals
...
class Animal {
    ...
    private String species;     // e.g. "Mallard Duck", "Holland Lop Rabbit"
    private String breed;
    private int ageMonths;
    private double price;
    private boolean available;
    private SourceType sourceType;
    private String breederName;  // populated when sourceType == BREEDER_RESALE
    private String healthStatus;

    // Constructor for farm-raised animals
    ...
    // Constructor for breeder resale animals
    ...
    /**
     * Marks the animal as sold.
     * @return true if sale succeeded, false if already sold
     */
    public boolean sell() {
        if (available) {
            available = false;
        ...
        System.out.println("Animal ID " + id + " is not available for sale.");
        return false;
    }
...
// FARM MANAGER CLASS - Central data manager
...
// MAIN GUI - Swing Interface

public class M3_JavaCode_GroupProject_02_WORKING_MejiasAndrea {
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            tabs.addTab("📊 Summary",   buildSummaryPanel(tabs));
    ...
    private static void showAddAnimalDialog() {
        ...
        String[] sources = {"Farm Raised", "Breeder Resale"};
    ...
    private static JPanel buildServicePanel() {
        ...
        payBtn.addActionListener(e -> {
            ...
            int id = (int) model.getValueAt(row, 0);
            Service s = manager.findService(id);
            if (s != null) { s.recordPayment(); refreshServiceTable(model);
    ...
    // SUMMARY PANEL
    private static JPanel buildSummaryPanel(JTabbedPane tabs) {
        JPanel panel = new JPanel(new BorderLayout(5, 5));
        panel.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
    ...
    private static void updateSummary(JTextArea text) {
        ...
        for (Animal a : manager.getAvailableAnimals())
    ...
    private static void seedData() {
        ...
        s4.complete(); // completed but not yet paid
...
// Claude.AI was used in a limited manner to help generate starter code to work with